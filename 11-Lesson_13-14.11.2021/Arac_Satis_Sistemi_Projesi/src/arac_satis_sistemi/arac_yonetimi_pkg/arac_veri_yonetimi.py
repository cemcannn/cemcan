...
def arac_ekle(arac:Arac): # Arac ekle fonksiyonu keyword arguments olarak arac parametresi Araç classı olarak tanımlanıyor.
    __arac_listesi[arac.benzersiz_kod]=arac # arac listesinin arac benzersiz kod indeksi arac parametresine eşitleniyor yani key olarak arac benzersiz kod value değerleri ise arac class ının geri kalan parametreleri oluyor.
...
def arac_getir_serino(serino:str) -> Arac: # Araç getir fonksiyonunu seri numaradan getiriyor.
    for arac in __arac_listesi.items(): # Buradaki arac_key ne işe yarıyor######################
